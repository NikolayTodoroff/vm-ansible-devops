...
The SSH key pair is created by Terraform so its lifecycle follows the infrastructure. Since `tls_private_key` stores the private key in Terraform state as plaintext, the state file is treated as sensitive and stored in an encrypted, RBAC-restricted remote Azure Storage backend.
...
`playbook.yml` installs nginx, deploys a static `index.html`, sets `ufw` rules for ports 22 and 80, and ensures nginx is running and enabled on boot.
